...
int axp22_probe(void)
{
	...
	if(pmu_type & 0x06)
	{
		...
		// setting PEK Key Parameters
		if(axp_i2c_read(AXP22_ADDR, BOOT_POWER22_POK_SET, &reg_value))
	    {
	        return -1;
	    }
	    reg_value &= ~(0x1<<3);// disable force shuting down
	    reg_value &= ~(0x3<<6);// set startup time to 128ms
		//if(reg_value)
		//{
	    if(axp_i2c_write(AXP22_ADDR, BOOT_POWER22_POK_SET, reg_value))
	    {
		return -1;
	    }
#if 0	    
