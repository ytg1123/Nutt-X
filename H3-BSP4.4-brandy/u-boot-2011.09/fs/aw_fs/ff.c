...
#endif
#define FAT_SIZE_AW (4*1024*1024)
#define FAT_FLAGS_AW (160)
static
WORD aw_pad_align;
static char fatfs_aw_fat_buffer[FAT_SIZE_AW];
static char *fatfs_aw_fat_buffer_p=fatfs_aw_fat_buffer;
static unsigned int aw_fat_flags[FAT_FLAGS_AW];
...
static
void aw_fat_sync(FATFS *fs)
{
	unsigned int init=0,tmp,min=0,max=0,i,j,will_write=0;
//	printf("%s enter fatfs_aw_fat_buffer_p 0x%p %d\n",__func__,fatfs_aw_fat_buffer_p,__LINE__);
	for(i=0;i<FAT_FLAGS_AW;i++){
		for(j=0;j<32;j++){
			if(aw_fat_flags[i]&(1<<j)){
				will_write = 1;
				tmp = i*32+j;
				if(!init){
					init =1;
				...
				if(tmp == max+1){
					...
					if (disk_write(fs->drv, (const BYTE *)(fatfs_aw_fat_buffer_p+min*512), fs->fatbase+min,max-min+1) != RES_OK)
						printf("sync err! min %d max %d  %d\n",min,max,__LINE__);
					if (disk_write(fs->drv, (const BYTE *)(fatfs_aw_fat_buffer_p+min*512), fs->fatbase + fs->fsize + min ,max-min+1) != RES_OK)
						printf("sync back err! min %d max %d  %d\n",min,max,__LINE__);
					min = tmp;
					max = tmp;
				}
			}
...
void aw_fat_sync()
{
	unsigned int init=0,tmp,min=0,max=0,i,j;
	for(i=0;i<FAT_FLAGS_AW;i++){
		for(j=0;j<32;j++){
			if(aw_fat_flags[i]&(1<<j)){
				tmp = i*32+j;
	...
	if (disk_write(fs->drv, (const char *)(fatfs_aw_fat_buffer_p+min*512), fp->fs->fatbase+min,max-min) != RES_OK)
		printf("sync err!\n %d",__LINE__);
}
#endif
...
#endif
		if (wsect >= fs->fatbase && wsect < (fs->fatbase + fs->fsize)) {	/* In FAT area? */
			mem_cpy(fatfs_aw_fat_buffer_p+ (wsect - fs->fatbase) * 512,fs->win , 512);
		//	printf("wsect 0x%x fatbase 0x%x diff %d %d\n",wsect,fs->fatbase,wsect - fs->fatbase,__LINE__);
			i = (wsect - fs->fatbase) / 32;
			j = (wsect - fs->fatbase) % 32;
			aw_fat_flags[i] |= (1 << j);
			for (nf = fs->n_fats; nf >= 2; nf--) {	/* Reflect the change to all FAT copies */
				wsect += fs->fsize;
#if 0
...
)
{
	if (sector != fs->winsect) {	/* Changed current window */
		...
		if (sync_window(fs) != FR_OK)
			return FR_DISK_ERR;
#endif
		if (disk_read_fs(fs->drv, fs->win, sector, 1) != RES_OK)
			return FR_DISK_ERR;
		fs->winsect = sector;
	}
...
)
{
	if (sector != fs->winsect) {	/* Changed current window */
		...
		if (disk_read_fs(fs->drv, fs->win, sector, 1) != RES_OK)
			return FR_DISK_ERR;
#endif
		mem_cpy (fs->win,fatfs_aw_fat_buffer_p + (sector - fs->fatbase) * 512 , 512);
		fs->winsect = sector;
	}
	return FR_OK;
}
...
/*-----------------------------------------------------------------------*/
/* aw FAT access - Read value of a FAT entry                             */
/*-----------------------------------------------------------------------*/


DWORD aw_get_fat (	/* 0xFFFFFFFF:Disk error, 1:Internal error, Else:Cluster status */
	FATFS *fs,	/* File system object */
	DWORD clst	/* Cluster# to get the link information */
)
{
	...
	if (clst < 2 || clst >= fs->n_fatent)	/* Check range */
		return 1;
	//printf("fs->fatbase 0x%x %d\n",fs->fatbase,__LINE__);
	switch (fs->fs_type) {
	case FS_FAT12 :
		bc = (UINT)clst; bc += bc / 2;
		if (aw_move_window(fs, fs->fatbase + (bc / SS(fs)))) break;
		wc = fs->win[bc % SS(fs)]; bc++;
		if (aw_move_window(fs, fs->fatbase + (bc / SS(fs)))) break;
		wc |= fs->win[bc % SS(fs)] << 8;
		return (clst & 1) ? (wc >> 4) : (wc & 0xFFF);
	...
	case FS_FAT16 :
		if (aw_move_window(fs, fs->fatbase + (clst / (SS(fs) / 2)))) break;
		p = &fs->win[clst * 2 % SS(fs)];
		return LD_WORD(p);

	case FS_FAT32 :
		if (aw_move_window(fs, fs->fatbase + (clst / (SS(fs) / 4)))) break;
		p = &fs->win[clst * 4 % SS(fs)];
		return LD_DWORD(p) & 0x0FFFFFFF;
	}
...
)
{
	...
	if (clst < 2 || clst >= fs->n_fatent) {	/* Check range */
		...
		case FS_FAT12 :
			bc = (UINT)clst; bc += bc / 2;
			res = aw_move_window(fs, fs->fatbase + (bc / SS(fs)));
			if (res != FR_OK) break;
			p = &fs->win[bc % SS(fs)];
			*p = (clst & 1) ? ((*p & 0x0F) | ((BYTE)val << 4)) : (BYTE)val;
			bc++;
		...
		case FS_FAT16 :
			res = aw_move_window(fs, fs->fatbase + (clst / (SS(fs) / 2)));
			if (res != FR_OK) break;
			p = &fs->win[clst * 2 % SS(fs)];
			ST_WORD(p, (WORD)val);
			break;
		...
		case FS_FAT32 :
			res = aw_move_window(fs, fs->fatbase + (clst / (SS(fs) / 4)));
			if (res != FR_OK) break;
			p = &fs->win[clst * 4 % SS(fs)];
			val |= LD_DWORD(p) & 0xF0000000;
			ST_DWORD(p, val);
...
#if !_FS_READONLY
static
DWORD aw_create_chain (	/* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
	FATFS *fs,			/* File system object */
	DWORD clst			/* Cluster# to stretch. 0 means create a new chain. */
)
{
	...
	if (clst == 0) {		/* Create a new chain */
		...
		cs = aw_get_fat(fs, clst);			/* Check the cluster status */
		if (cs < 2) return 1;			/* It is an invalid cluster */
		if (cs < fs->n_fatent) return cs;	/* It is already followed by next cluster */
		scl = clst;
	}
	...
	for (;;) {
		...
		cs = aw_get_fat(fs, ncl);			/* Get the cluster status */
		if (cs == 0) break;				/* Found a free cluster */
		if (cs == 0xFFFFFFFF || cs == 1)/* An error occurred */
			return cs;
		if (ncl == scl) return 0;		/* No free cluster */
	...
	res = aw_put_fat(fs, ncl, 0x0FFFFFFF);	/* Mark the new cluster "last link" */
	if (res == FR_OK && clst != 0) {
		res = aw_put_fat(fs, clst, ncl);	/* Link it to the previous one if needed */
	}
	if (res == FR_OK) {
		fs->last_clust = ncl;			/* Update FSINFO */
		if (fs->free_clust != 0xFFFFFFFF) {
...
{
	if (disk_read_fs(fs->drv, fs->win, sect, 1) != RES_OK)	/* Load boot record */
		return 3;
...
)
{
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	if (fmt == FS_FAT32) {
		...
		if (disk_read_fs(fs->drv, fs->win, fs->fsi_sector, 1) == RES_OK &&
...
#endif
	aw_pad_align =1;
//	printf("fs->n_fats %d FAT_SIZE  0x%x %d\n",fs->n_fats,sizeof(fatfs_aw_fat_buffer),__LINE__);
	if (disk_read_fs(fs->drv, (BYTE *)fatfs_aw_fat_buffer_p, fs->fatbase,  fs->fsize) != RES_OK)
		printf("%s read fat err! %d\n",__func__,__LINE__);
#if 0
	mem_cpy(fatfs_aw_fat_back_buffer_p,fatfs_aw_fat_buffer_p,fs->fsize);
	if (disk_read_fs(fs->drv, fatfs_aw_fat_back_buffer_p, fs->fatbase + fs->fsize,fs->fsize) != RES_OK)
		printf("%s read fat err! %d\n",__func__,__LINE__);
#endif
	return FR_OK;
}
...
{
	FILE *fil = (FILE*)obj;	/* Assuming offset of fs and id in the FILE/DIR is identical */
...
)
{
	...
	if (part_name)
	{
		memset(PART_NAME[vol], 0, 16);
		strcpy(PART_NAME[vol], part_name);
	}
	else
	{
		memset(PART_NAME[vol], 0, 16);
...
)
{
	...
	ret = sunxi_partition_get_name(part_index, part_name);
	if(ret < 0)
	{
		printf("fs mount err: unable to get part index %d\n", part_index);

		return -1;
	}
	printf("mount part name %s\n", part_name);

	return f_mount(vol, fs, part_name);
}
...
)
{
	...
	BYTE *mybuff =  (BYTE*)buff;
	DWORD will_read,will_read_start=0;
	DWORD debug_size;
	DWORD read_write_data;
//	tick_printf(__FILE__, __LINE__);
	*br = 0;	/* Clear read byte counter */
	...
	for ( ;  btr;								/* Repeat until all data read */
		rbuff += rcnt, fp->fptr += rcnt, *br += rcnt) {
...
#endif
				if (disk_read_fs(fp->fs->drv, fp->buf, sect, 1) != RES_OK)	/* Fill sector cache */
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
)
{
	...
	BYTE csect;
	BYTE *mybuff =  (BYTE*)buff;
	DWORD will_write=1,will_write_start=0,read_write_data;
	DWORD debug_size;
...
//	printf("%s enter %d\n",__func__,__LINE__);
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		wbuff += wcnt, fp->fptr += wcnt, *bw += wcnt) {
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			csect = (BYTE)(fp->fptr / SS(fp->fs) & (fp->fs->csize - 1));	/* Sector offset in the cluster */
			if (!csect) {					/* On the cluster boundary? */
				if (fp->fptr == 0) {		/* On the top of the file? */
...
#endif
						fp->sclust = clst = aw_create_chain(fp->fs, 0);	/* Create a new cluster chain */
				} else {					/* Middle or end of the file */
#if _USE_FASTSEEK
					if (fp->cltbl)
						clst = clmt_clust(fp, fp->fptr);	/* Get cluster# from the CLMT */
...
#endif
						clst = aw_create_chain(fp->fs, fp->clust);	/* Follow or stretch cluster chain on the FAT */
				}
			...
			if (fp->flag & FA__DIRTY) {		/* Write-back sector cache */
				if (disk_write(fp->fs->drv, fp->buf, fp->dsect, 1) != RES_OK)
					ABORT(fp->fs, FR_DISK_ERR);
//				printf("%s  dsect  0x%x  %d\n",__func__,fp->dsect,__LINE__);
				fp->flag &= ~FA__DIRTY;
			}
#endif
			sect = clust2sect(fp->fs, fp->clust);	/* Get current sector */
			...
			if (cc) {						/* Write maximum contiguous sectors directly */
				if (csect + cc > fp->fs->csize)	/* Clip at cluster boundary */
					cc = fp->fs->csize - csect;
				if(!will_write_start){
					will_write_start = sect;
					read_write_data	 = cc;
					debug_size = cc;
				}else if((will_write_start + read_write_data) == sect){
					read_write_data += cc;
					debug_size += cc;
				}else{
...
//					printf("%s write over !%d",__func__,__LINE__);
					if (disk_write(fp->fs->drv, mybuff, will_write_start, read_write_data) != RES_OK){
						...
						will_write = 0;
//						printf("%s over write will_write_start 0x%x read_write_data %d %d\n",__func__,will_write_start,read_write_data,__LINE__);
				}
				continue;
			}
#if _FS_TINY
			...
			if (fp->dsect != sect) {		/* Fill sector cache with file data */
				if (fp->fptr < fp->fsize &&
					disk_read_fs(fp->fs->drv, fp->buf, sect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
//				printf("%s over write will_write_start 0x%x read_write_data %d %d\n",__func__,will_write_start,read_write_data,__LINE__);
//				printf("%s write over1 !%d",__func__,__LINE__);
				will_write = 0;
		}
	}
...
//	printf("%s write over2 !%d",__func__,__LINE__);
	LEAVE_FF(fp->fs, FR_OK);
}
...
)
{
	...
	aw_fat_sync(fp->fs);
#if 0
	if (disk_write(fp->fs->drv, fatfs_aw_fat_buffer_p, fp->fs->fatbase,fp->fs->fsize) != RES_OK)
		printf("%s write fat err! %d\n",__func__,__LINE__);
	if (disk_write(fp->fs->drv, fatfs_aw_fat_buffer_p, fp->fs->fatbase + fp->fs->fsize,fp->fs->fsize) != RES_OK)
		printf("%s write fat err! %d\n",__func__,__LINE__);
#endif
#if _FS_READONLY
	res = validate(fp);
...
#endif
					if (disk_read_fs(fp->fs->drv, fp->buf, dsc, 1) != RES_OK)	/* Load current sector */
						ABORT(fp->fs, FR_DISK_ERR);
#endif
					fp->dsect = dsc;
				}
...
#endif
			if (disk_read_fs(fp->fs->drv, fp->buf, nsect, 1) != RES_OK)	/* Fill sector cache */
				ABORT(fp->fs, FR_DISK_ERR);
#endif
			fp->dsect = nsect;
		}
...
)
{
	...
	if (_MULTI_PARTITION && part) {
		...
		if (disk_read_fs(pdrv, fs->win, 0, 1) != RES_OK) return FR_DISK_ERR;
