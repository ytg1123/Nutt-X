...
void set_i2c_clock(void)
{
	...
	__msdelay(1);
#endif
	reg_value = readl(CCM_APB2_GATE0_CTRL);
	reg_value &= ~(1<<bus_num);
	writel(reg_value, CCM_APB2_GATE0_CTRL);
	__msdelay(1);
	reg_value |= (1 << bus_num);
	writel(reg_value, CCM_APB2_GATE0_CTRL);
}
...
void i2c_init(int speed, int slaveaddr)
{
	...
	normal_gpio_set_t twi_gpio[2] = {
		//twi1_sck= port:PH4<2><default><default><default>
		//twi1_sda= port:PH5<2><default><default><default>
		{ 8, 4, 2, -1, -1, -1, {0}},
		{ 8, 5, 2, -1, -1, -1, {0}}
	};
