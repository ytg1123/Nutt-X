...
LIBS += net/libnet.o
LIBS += drivers/efuse/libsunxi_efuse.o
...
spl_lib: $(TIMESTAMP_FILE) $(VERSION_FILE) depend
		$(MAKE) -C $(SPLBASE)/sunxi_spl/spl all
...
sboot:  spl_lib depend
		...
		@if [ -z "$(findstring $(OTA_TEST_NAME), $(SUNXI_MODE))" ]; then \
			...
			cp -v sboot_$(CONFIG_TARGET_NAME).bin $(TOPDIR)/../../../../target/allwinner/$(TARGET_PLATFORM)-common/bin/sboot_$(CONFIG_TARGET_NAME)-$(SUNXI_MODE).bin; \
		fi
...
clean:
	...
	@rm -f $(obj)board/cray/L1/{bootscript.c,bootscript.image}	  \
	       ...
	       $(obj)$(SPLBASE)/sunxi_spl/boot0/boot0.lds      		  \
	       $(obj)$(SPLBASE)/sunxi_spl/fes_init/fes_init.lds      	  \
