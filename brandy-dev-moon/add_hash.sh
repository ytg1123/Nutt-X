...
build_boot0()
{
    dd if=${BUILD_FILE} of=./boot0_back bs=760 count=1 status=noxfer
    dd if=./cur.log  of=./boot0_back ibs=64 conv=notrunc,sync oflag=append obs=64 count=1 status=noxfer
    dd if=${BUILD_FILE} of=./boot0_back ibs=824 obs=824 conv=notrunc oflag=append skip=1 status=noxfer
