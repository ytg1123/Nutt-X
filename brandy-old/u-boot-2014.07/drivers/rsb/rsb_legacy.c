...
s32 sunxi_rsb_init(u32 slave_id)
{
	if(gd->securemode != SUNXI_SECURE_MODE_NO_SECUREOS)
	{
		...
		//printf("normal mode or secure mode without secure os\n");
		rsb_init();
	...
	{
		printf("rsb init with secure os\n");
	}
...
/**
 * config rsb: allocate a runtime rsbadrress.don't use this function twoice 
 * @slave_id: chip_id
 * @rsb_addr: rbs address.
 */
static s32 sunxi_rsb_config_secos(u32 slave_id, u32 rsb_addr)
{
	u32 rtaddr 		= 0;
	...
	rsb_printk("[PMU]: ""%s: %d:\n"\
				"[PMU]: don't conifg the pmu twice when has one pmu.\n", \
					__FILE__, __LINE__);
