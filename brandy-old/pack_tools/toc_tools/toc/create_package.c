...
int create_package(toc_descriptor_t *package, char *package_name)
{
	...
	for(p_item_head=item_head;content_count > 0;p_item_head++, p_package++, content_count--)
	{
		...
		fclose(src_file);
		src_file = NULL;
		struct spare_boot_ctrl_head  *file_head = (struct spare_boot_ctrl_head *)(package_content + offset);
		...
		if (!strcmp(p_item_head->name, "monitor")) {
			p_item_head->run_addr = file_head->run_addr;
		} else if(!strcmp(p_item_head->name, "u-boot")) {
			p_item_head->run_addr = file_head->run_addr;
		} else if(!strcmp(p_item_head->name, "optee")) {
			p_item_head->run_addr = file_head->run_addr;
		}
