...
int update_for_boot0(char *boot0_name, int storage_type)
{
	...
	if(!script_parser_mainkey_get_gpio_cfg("pwr_ctrl", gpio_set, 32))
	{
		if(gpio_set[1].port)
		{
			printf("update boot0, pwr_ctrl GPIO:P%c%d\n", gpio_set[1].port-1+'A', gpio_set[1].port_num);
			simple_gpio_cfg* pwr_simple_gpio = boot0_head->prvt_head.reserve;
			pwr_simple_gpio->port      = gpio_set[1].port;
			pwr_simple_gpio->port_num  = gpio_set[1].port_num;
		}
	}
	//取出数据进行修正，NAND参数
	if(!storage_type)
	...
	else if(1 == storage_type) //取得卡参赛
	{
		...
		if(!script_parser_fetch("card_boot", "card_no", value))
		{
			printf("get card_no from card_boot, set card_no = %d\n", value[0]);
			boot0_head->boot_head.platform[0]  = value[0];
		}
		else
		{
			//get real storage type from sys_config
			if(!script_parser_fetch("target", "storage_type", value))
			{
				if(value[0] == 6)  //STORAGE_SD1
				{
					printf("get storage_type 6, it's sd1, set card_no = 1\n");
					boot0_head->boot_head.platform[0] = 1;
				}
			}
		}
		printf("set boot0_head->boot_head.platform[0] to %d\n",	boot0_head->boot_head.platform[0]);
