...
#ifdef CONFIG_READ_LOGO_FOR_KERNEL
/* return:0(get the flag); -1(can't get the flag) */
int read_recoverylogo_flag(void)
{
	int flag = -1;
	int recoverylogo_used = 0;

	if (script_parser_fetch("recoverylogo", "recoverylogo_used", &recoverylogo_used, 1)) {
		return flag;
	}
	if (recoverylogo_used == 0) {
		return flag;
	}
	...
	flag = sunxi_check_one_str(boot_commond,"boot_recovery");
	return flag;
}
#endif
...
/* #endif */ /* CONFIG_GENERIC_MMC*/
	...
	if (workmode == WORK_MODE_BOOT) {
#if defined(CONFIG_SUNXI_SCRIPT_REINIT)
		{
			...
			ret = f_open(&script_fs, "0:script.bin",
			...
			if (ret)
				...
				if (!f_read(&script_fs, buf, 100 * 1024,
								&read_bytes)) {
...
#endif /* (defined(CONFIG_SUNXI_DISPLAY) || defined(CONFIG_SUN7I_DISPLAY)) */

#ifdef CONFIG_READ_LOGO_FOR_KERNEL
		if(read_recoverylogo_flag()) {
			sunxi_read_bootlogo("bootlogo");
		} else {
			sunxi_read_bootlogo("recoverylogo");
		}
#endif
