...
void *g_tone_buffer;
static sunxi_dma_setting_t *g_codec_tx_buf;
...
static int sunxi_boot_tone_dma_init(void)
{
	...
	g_codec_tx_buf = malloc_noncache(sizeof(sunxi_dma_setting_t));
	memset(g_codec_tx_buf, 0, sizeof(sunxi_dma_setting_t));
	...
	g_codec_tx_buf->cfg.src_drq_type	= DMAC_CFG_TYPE_DRAM;
	g_codec_tx_buf->cfg.src_addr_mode	= DMAC_CFG_SRC_ADDR_TYPE_LINEAR_MODE;
	g_codec_tx_buf->cfg.src_burst_length	= DMAC_CFG_SRC_1_BURST;
	g_codec_tx_buf->cfg.src_data_width	= DMAC_CFG_SRC_DATA_WIDTH_16BIT;
	g_codec_tx_buf->cfg.reserved0		= 0;

	g_codec_tx_buf->cfg.dst_drq_type	= DMA_CFG_SRC_DRQ_CODEC;
	g_codec_tx_buf->cfg.dst_addr_mode	= DMAC_CFG_DEST_ADDR_TYPE_IO_MODE;
	g_codec_tx_buf->cfg.dst_burst_length	= DMAC_CFG_DEST_4_BURST;
	g_codec_tx_buf->cfg.dst_data_width	= DMAC_CFG_DEST_DATA_WIDTH_16BIT;
	g_codec_tx_buf->cfg.reserved1		= 0;

	/* g_codec_tx_buf->cfg.wait_state		= 4; */
	/* g_codec_tx_buf->cfg.continuous_mode	= 0; */
...
static int boot_tone_is_enable(u32 *limit)
{
	...
	ret = script_parser_fetch("boottone", "boottone_used", &val, 1);
	if (ret != 0) {
		printf("unable to find boottone node in sys_config.\n");
		return 0;
	}
	if (val != 1) {
		printf("boottone disabled in sys_config\n");
		return 0;
	}
	ret = script_parser_fetch("boottone", "len_limit", &val, 1);
	if (!ret)
		*limit = val;
...
int sunxi_boot_tone_play(void)
{
	...
	g_tone_buffer = (void *)simple_strtoul(getenv("uboot_tone_addr"), NULL, 16);
	if (!g_tone_buffer)
		return 0;
	ret = run_command(getenv("load_boottone"), 0);
	if (ret >= 0)
