...
static int get_sunxi_codec_values(struct sunxi_codec *codec)
{
	int ret, val;

	ret = script_parser_fetch("audio0", "audio_used", &val, 1);
	if (ret != 0) {
		printf("parser audio0 failed\n");
		return -1;
	}
	if (val != 1) {
		printf("audio0 not used\n");
		return -1;
	}

	codec->addr_clkbase = (void __iomem *)_ADDA_CCU_BASE;
	codec->addr_dbase = (void __iomem *)CODEC_BASSADDRESS;
	codec->addr_abase = (void __iomem *)ADDA_PR_CFG_REG_ADDRESS;

	ret = script_parser_fetch("audio0", "headphone_vol", &val, 1);
	if (ret != 0)
		val = 0x3b;
	codec->headphone_vol = val;
	ret = script_parser_fetch("audio0", "lineout_vol", &val, 1);
	if (ret != 0)
		val = 0x10;
	codec->lineout_vol = val;


	ret = script_parser_fetch("audio0", "audio_pa_ctrl",
			(void *)&codec->spk_cfg, sizeof(user_gpio_set_t)>>2);
	...
	ret = script_parser_fetch("audio0", "pa_ctl_level", &val, 1);
	if (ret != 0)
		val = 1;
	codec->pa_ctl_level = val;
...
int sunxi_codec_probe(void)
{
	...
	if (get_sunxi_codec_values(codec) < 0) {
