...
*    return        :
*
*    note          :  寻找目标字符串dest_buf(格式：空格隔开，比如"AAA BBB C DD EEE"，但是不包括tab
*                     中是否有goal子字符串,有返回0,没有返回-1
************************************************************************************************************
*/
int sunxi_check_one_str(char *dest_buf, char *goal)
{
	char tmp[128];
	char tmp_str[16];
	int  dest_len;
	...
	for(i=0;tmp[i];)
	{
		...
		{
			printf("Get str goal(\"%s\") from dest(\"%s\") \n", goal, dest_buf);
			return 0;
		}
	}
